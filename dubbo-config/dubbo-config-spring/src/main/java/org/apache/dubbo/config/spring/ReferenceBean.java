...
// ReferenceBean的定义
// 1、实现ApplicationContextAware，会自动初始化Spring上下文
// 2、实现FactoryBean，当引用对象的时候，会调用getObject();
// 3、实现InitializingBean，对象属性填充后，会调用afterPropertiesSet();
// 4、实现DisposableBean，销毁时，会调用destroy()。
// 所以，服务引用的触发点有两个，
